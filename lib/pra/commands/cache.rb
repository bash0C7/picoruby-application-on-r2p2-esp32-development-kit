    ...
    class Cache < Thor
      ...
      def list
        ...
        %w[R2P2-ESP32 picoruby-esp32 picoruby].each do |repo|
          repo_cache = File.join(Pra::Env::CACHE_DIR, repo)
      ...
      def fetch(env_name = 'latest')
        ...
        # キャッシュディレクトリを作成
        FileUtils.mkdir_p(Pra::Env::CACHE_DIR)

        # 各リポジトリをキャッシュに保存
        repos_to_fetch = [
          { repo: 'R2P2-ESP32', url: Pra::Env::REPOS['R2P2-ESP32'], commit: env_config['R2P2-ESP32']['commit'] },
          { repo: 'picoruby-esp32', url: Pra::Env::REPOS['picoruby-esp32'],
            commit: env_config['picoruby-esp32']['commit'] },
          { repo: 'picoruby', url: Pra::Env::REPOS['picoruby'], commit: env_config['picoruby']['commit'] }
        ]

        repos_to_fetch.each do |repo_info|
          repo_name = repo_info[:repo]
          cache_hash = repo_info[:commit] + '-' + env_config[repo_name]['timestamp']
          cache_path = Pra::Env.get_cache_path(repo_name, cache_hash)

          if Dir.exist?(cache_path)
            puts "  ✓ #{repo_name} already cached"
          else
            puts "  Fetching #{repo_name}..."
            Pra::Env.clone_with_submodules(repo_info[:url], cache_path, repo_info[:commit])
            puts "  ✓ #{repo_name} cached to #{cache_path}"
          end
        end

        # R2P2-ESP32からsubmodule情報を取得して検証
        r2p2_cache = Pra::Env.get_cache_path('R2P2-ESP32',
                                             env_config['R2P2-ESP32']['commit'] + '-' + env_config['R2P2-ESP32']['timestamp'])

        _info, warnings = Pra::Env.traverse_submodules_and_validate(r2p2_cache)
      ...
      def clean(repo)
        cache_path = File.join(Pra::Env::CACHE_DIR, repo)
        if Dir.exist?(cache_path)
      ...
      def prune
        ...
        # キャッシュディレクトリをスキャン
        %w[R2P2-ESP32 picoruby-esp32 picoruby].each do |repo|
          repo_cache_dir = File.join(Pra::Env::CACHE_DIR, repo)
