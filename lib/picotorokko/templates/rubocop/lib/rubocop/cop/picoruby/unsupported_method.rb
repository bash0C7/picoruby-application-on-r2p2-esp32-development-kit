    ...
    module PicoRuby
      ...
      class UnsupportedMethod < Base
        ...
        def find_data_file(filename)
          ...
          possible_paths = [
            # .ptrk_env/<current_env>/rubocop/data (generated by ptrk env set)
            ptrk_env_data_path(filename),
        ...
        def ptrk_env_data_path(filename)
          env_file = File.join(Dir.pwd, '.ptrk_env', '.picoruby-env.yml')
          return nil unless File.exist?(env_file)

          require 'yaml'
          env_config = YAML.load_file(env_file)
          current_env = env_config['current']
          return nil unless current_env

          File.join(Dir.pwd, '.ptrk_env', current_env, 'rubocop', 'data', filename)
